...
const handleTestNetwork = async (i: number) => {
  if (i <= networkList.list.length - 1) {
    ...
    if (networkList.list[i].onLine) {
      console.log(util.getCache('networkId'));
      console.log(networkList.list[i].networkId);
      const networkId = util.getCache('networkId');
      if (networkId) {
        if (util.getCache('networkId') === networkList.list[i].networkId) {
...
const handleConnectInner = async (row: networkLogin) => {
  console.log(row.ip);
  const list = util.getCache('networks');
  const obj = list.find((item: networkLogin) => row.ip === item.ip);
  console.log(obj);
  util.setCache('networkId', obj.networkId);
  handleTestNetwork(0);
};
